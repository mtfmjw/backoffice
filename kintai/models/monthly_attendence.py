...
class MonthlyAttendance(BaseModel):
    ...
    actual_work_minutes = models.PositiveIntegerField(_("実労働時間（分）"), null=True, blank=True)
    overtime_minutes = models.PositiveIntegerField(_("残業時間（分）"), null=True, blank=True)
    night_work_minutes = models.PositiveIntegerField(_("深夜労働時間（分）"), null=True, blank=True)
    worked_days = models.PositiveIntegerField(_("出勤日数"), null=True, blank=True)
    paid_leave_days = models.PositiveIntegerField(_("有給休暇日数"), null=True, blank=True)  # 半休含めて
